...
export function ArchitectureSidebarGraph({
  ...
  const viewId = useId();
  const viewTabRefs = useRef<Record<'layers' | 'graph', HTMLButtonElement | null>>({
    layers: null,
    graph: null,
  });
      ...
      <div className="components-section-header">
        ...
        >
          ...
          <span className="section-count">
            {nodes.filter((node) => !node.data.isAnchor).length}
          </span>
          <span className="section-toggle-icon" aria-hidden="true">
            {expanded ? <FiChevronDown /> : <FiChevronsRight />}
          </span>
        </button>
        <div className="sidebar-component-view-switcher">
          ...
          >
            <button
              ...
              onKeyDown={(event) => selectViewFromKeyboard(event, 'layers')}
            >
              <FiList /> Layers
            </button>
            <button
              ...
              onKeyDown={(event) => selectViewFromKeyboard(event, 'graph')}
            >
              <FiGitBranch /> Graph
            </button>
          </div>
        ...
        (view === 'layers' ? (
          ...
          >
            {groups.map(({ kind, title, items }) => {
              ...
              return (
                <section className="sidebar-component-tree__group" key={kind}>
                  <button
            ...
            <div className="sidebar-topology-graph__content" style={{ height: layout.height }}>
              ...
              >
                {layout.edges.map(({ id, source, target, channelX }) => {
                  ...
                  return (
                    <path key={id} d={getGitRailPath(source.x, startY, target.x, endY, channelX)} />
                  );
                })}
                {layout.nodes.map(({ node, x, y }) => (
                  <circle
                    key={node.id}
                    className={`sidebar-topology-graph__commit sidebar-topology-graph__commit--${connectionStates.get(node.id)?.state ?? 'isolated'} ${node.selected ? 'sidebar-topology-graph__commit--selected' : ''}`}
                    cx={x}
