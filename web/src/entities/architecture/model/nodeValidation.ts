...
export function validateArchitectureNodes(
  ...
  const selfConnectedNodeIds = new Set(edges.filter((edge) => edge.source === edge.target).map((edge) => edge.source));
...
export function getArchitectureNodeValidationIssues(states: Map<string, ArchitectureNodeValidationState>) {
