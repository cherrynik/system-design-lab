...
describe('node validation', () => {
  it('accepts a complete request path', () => {
    const nodes = [node('client', 'client', 'Client'), node('balancer', 'load-balancer', 'Load Balancer'), node('service', 'service', 'Service')];
    const states = validateArchitectureNodes(nodes, [edge('one', 'client', 'balancer'), edge('two', 'balancer', 'service')]);
  ...
  it('reports the missing direction for each isolated component', () => {
    ...
    expect(states.get('balancer')?.issues.map(({ code }) => code)).toEqual(['NODE_INPUT_REQUIRED', 'NODE_OUTPUT_REQUIRED']);
  ...
  it('treats a self connection as an error', () => {
    ...
    const state = validateArchitectureNodes([service], [edge('loop', 'service', 'service')]).get('service');

    expect(state?.status).toBe('error');
    expect(state?.issues).toEqual([expect.objectContaining({ code: 'NODE_SELF_CONNECTION', severity: 'error' })]);
  });

  it('does not validate free arrow anchors as components', () => {
    const states = validateArchitectureNodes([
      node('client', 'client', 'Client'),
      node('anchor', 'service', '', true),
