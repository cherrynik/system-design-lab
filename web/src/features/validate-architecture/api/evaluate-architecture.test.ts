...
describe('toArchitecturePayload', () => {
  ...
  it('keeps typed components and bound directed arrows only', () => {
    expect(
      toArchitecturePayload(components, [{ source: 'client-1', target: 'service-1' }]),
    ).toEqual({
      nodes: [
  ...
  it('ignores arrows that are not bound at both ends', () => {
    expect(
      toArchitecturePayload(components, [{ source: 'client-1', target: 'missing' }]).edges,
    ).toEqual([]);
  });

  it('removes deleted nodes and their connections from the architecture', () => {
    expect(
      toArchitecturePayload(components.slice(0, 1), [{ source: 'client-1', target: 'service-1' }]),
    ).toEqual({
  ...
  it('keeps free drawing anchors out of the validation payload', () => {
    const nodes = [
      ...components,
      { id: 'anchor-1', data: { kind: 'client' as const, isAnchor: true } },
    ];
